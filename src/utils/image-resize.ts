...
export default async function imageResizer(file: Blob, business_name: String) {
  ...
  const imageToUpload = new Blob([resizedImage.toBuffer()]).slice(
    0,
    resizedImage.size,
    "image/jpg",
  );
  ...
  return supabaseClient.storage
    ...
    .upload(`business_images/${file_name}.jpg`, imageToUpload, {
      cacheControl: "3600",
      upsert: false,
    })
    .then((result) => {
      const image_url =
        process.env.SUPABASE_IMAGE_PREFIX_URL! + file_name + ".jpg";
